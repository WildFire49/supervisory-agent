...
def mcp_tool_node(state: AgentState):
    print("---MCP TOOL CALL---")
    tool_json = state.get('mcp_tool_json')
    if not tool_json:
        state['response'] = {"error": "Router decided to call a tool but didn't provide the details."}
        return state
        
    result = mcp_client.call_tool(tool_json)
